...
export default function CustomMarker({
	...
	// Lås shape per fordon för att undvika att markören "hoppar" .
	// Byt bara till ny shape om den nya ligger nära RT-positionen.
	const shapePoints = useMemo(() => {
		const locked = lockedShapeRef.current;

		// Om vi temporärt saknar shape, behåll senast låsta shape
		...
		if (!locked) {
			lockedShapeRef.current = {
				shapeId: incomingShapeId,
				points: incomingShapePoints,
			};
			...
			>
				...
				>
					...
					>
						...
						<span className="line-text__destination">{currentDestination}</span>
					</span>
				</div>
			</AdvancedMarker>
			{isActive && (
